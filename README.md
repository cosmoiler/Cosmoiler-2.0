...
### Отладка: принудительная связь (`FORCE_CONNECTED`)

Большая часть интерфейса спрятана за флагом связи: без блока неактивны вкладки
«Телеметрия» и «Настройки» (их id задаёт `pages/appview.svelte` по флагу
`connected`), страницы показывают только надпись «Нет связи», кнопки заблокированы.
Отлаживать вёрстку в таких условиях нельзя.

Поэтому в `src/js/store.js` есть переключатель:

```js
const FORCE_CONNECTED = import.meta.env.DEV;
```

`import.meta.env.DEV` подставляет Vite: `true` при `npm run dev`, `false` при
`npm run build`. Когда флаг включён, `startWatchdog()` не запускает сторож вовсе,
а `state.connect` сразу становится `true` — заодно пропадает шум от зондов `/ping`
к отсутствующему устройству.

⚠️ **В продакшене ничего не остаётся**: при `npm run build` ветка сворачивается
сборщиком целиком. Это проверяется одной командой — в собранном `www/app.js` не
должно быть ни `FORCE_CONNECTED`, ни `import.meta.env`:

```powershell
node -e "const s=require('fs').readFileSync('www/app.js','utf8');console.log(s.includes('FORCE_CONNECTED'), s.includes('import.meta.env'))"
```

⚠️ Флаг обязан оставаться привязанным к `import.meta.env.DEV`. Если выставить
`true` руками — отладка уедет в прошивку.

⚠️ Принудительная связь **не подменяет данные**: настроек с устройства нет, поэтому
страницы покажут значения по умолчанию из `store.js`, а команды записи не пройдут
(запросов к устройству никто не отменял). Лечится это только реальным блоком
в той же сети — адрес задаётся параметром `?ws=`.
